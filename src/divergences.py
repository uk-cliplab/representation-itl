...
import matrix_itl_torch_approx as approx
...
def divergenceJR_unweighted(X,Y,sigma,alpha):
    ...
    Hj = itl.matrixAlphaJointEntropy([K, Kl_], alpha=alpha)
    Hl = approx.matrixAlphaEntropyLabel(L, alpha)
    divergence  = Hxy + Hl - Hj
    return divergence

def divergenceJR_rff(X,Y,sigma,alpha, n_rff):
    # Getting number of samples from each class
    N = X.shape[0]
    M = Y.shape[0]
    # Creating indicator or label variable
    ...
    cov_y = approx.cov_matrix_rff(Y,sigma,D,random = False)
